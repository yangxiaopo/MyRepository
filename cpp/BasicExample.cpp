...
void BasicExample::initPhysics()
{
...

	{
		...
		int result = wobj.loadObj("../../data/bunny.obj");
		if (!result)
		{
			//ERROR
		}
		...
		btTriangleIndexVertexArray* colonVertexArrays = new btTriangleIndexVertexArray(
			...
			3 * sizeof(int),
			wobj.mVertexCount,
			(btScalar*)wobj.mVertices,
			3 * sizeof(float)
		);

		btGImpactMeshShape* bunnymesh = new btGImpactMeshShape(colonVertexArrays);
		m_collisionShapes.push_back(bunnymesh);
		bunnymesh->setLocalScaling(btVector3(0.5f, 0.5f, 0.5f));
		bunnymesh->updateBound();*/
		//////////////////////////////////////////////////////////////////////////////////////
		//”√¥À”–ŒÛ≤Ó
		//btConvexHullShape* convexHullShape = new btConvexHullShape((const btScalar*)wobj.mVertices, wobj.mVertexCount, sizeof(float));

		btConvexHullShape* convexHullShape = new btConvexHullShape();
		for (int i = 0; i < wobj.mVertexCount; i++)
		{
			btVector3 vtx(wobj.mVertices[i * 3], wobj.mVertices[i * 3 + 1], wobj.mVertices[i * 3 + 2]);
