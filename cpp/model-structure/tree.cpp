...
template<class Key,class Value>
typename Tree<Key,Value>::Tree_node* Tree<Key,Value>::iterative_search(Key_type v)
{
	while(root != 0 && v != root->key)
	{
		if(v < root->key)
			root = root->left;
		else
